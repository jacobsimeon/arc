...
module Arc
  module Schemas
    ...
    describe Schema::Table do
      it 'includes Collector' do
        Schema::Table.included_modules.should include(Collector)
      end
      it 'aliases item_names to column_names' do
        table = Schema::Table.new nil, :superheros
        table.should respond_to(:column_names)
      end
      describe '#new' do
        it 'has a name' do
          Schema::Table.new('superheros').name.should == 'superheros'
        end
        it 'sets the data store' do
          ArcTest.with_store do |store|
            Schema::Table.new('superheros', store).instance_variable_get(:@data_store).should be(store)
          end
        end
      end
    end
